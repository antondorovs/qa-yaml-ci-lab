...
|-- examples/api-deprecation-policy.yaml
...
API deprecation policies must provide adequate notice, parallel compatibility,
consumer evidence and monitored approval before an old version is shut down.
...
to approved critical hotfixes and require smoke and observability checks before
...
counts, repository paths and validation errors without machine-specific absolute
paths. It also lists affected files so CI output can identify the scope of a
failed validation quickly.
